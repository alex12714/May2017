...
//            $dy.=" ((b.check_in BETWEEN '$this->check_in' AND '$this->check_out') OR (b.check_out BETWEEN '$this->check_out' AND '$this->check_in')) And";
        ...
        $RES = "invalid";
        $qaa = "SELECT a.*,b.*,c.*,d.* FROM arib_property_tbl a LEFT JOIN arib_space_tbl b ON a.property_id=b.property_id LEFT JOIN arib_property_address_tbl c ON b.property_id=c.property_id LEFT JOIN  arib_property_price_tbl d ON c.property_id=d.property_id WHERE $dy a.added_status='1' group by a.property_id";
        $ObjDB = new class_db();
...
//            $dy.=" ((b.check_in BETWEEN '$this->check_in' AND '$this->check_out') OR (b.check_out BETWEEN '$this->check_out' AND '$this->check_in')) And";
        ...
        $RES = "invalid";
        $query = "SELECT a.*,b.*,c.*,d.* FROM arib_property_tbl a LEFT JOIN arib_space_tbl b ON a.property_id=b.property_id LEFT JOIN arib_property_address_tbl c ON b.property_id=c.property_id LEFT JOIN  arib_property_price_tbl d ON c.property_id=d.property_id WHERE $dy a.added_status='1' group by a.property_id";
        $ObjDB = new class_db();
    ...
    public function wishlistpropertydeatils() {
        ...
        $RES = "invalid";
        $qaa = "SELECT a.*,b.* FROM arib_property_tbl a LEFT JOIN arib_wishist_tbl b ON a.property_id=b.property_id  WHERE b.guest_id='$this->guest_id' AND wishist_category_id='$this->wishist_category_id' AND a.added_status='1'  GROUP BY a.property_id ORDER BY a.timesatmp DESC";
        $ObjDB = new class_db();
        $ObjDB->sproc_name = $qaa;
        $RES = $ObjDB->SelectQuery();
        return $RES;
